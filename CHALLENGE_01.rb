...
def decode_message message
  ...
  formated_message.each do |m|
    unless $memo.any? { |el| el.include?(m) } # It reads as: unless an element of memeo doesn't contains the word
      $decoded_message.push("#{m.concat(formated_message.select { |mes| mes === m }.count.to_s)}") # Save the word with the number of times it repeats in the message
      $memo.push(m) # Save the message in memo to save time and memeory
    end
  end

  $decoded_message.join("") # Finally, join the decoded_message to get a single string
end

# Convert the string into an array separated by the space. Map the array and change every word to lowercase
